...
function totalOrderValue(orders, applyDiscount) {
  // Step 1: map each order to its discounted total value
  const orderTotals = orders.map(order => {
    ...
    const finalTotal = order.discountCode
      ? applyDiscount(order.discountCode, totalBeforeDiscount)
      : totalBeforeDiscount;

    return finalTotal;
  });

  // Step 2: reduce all discounted totals into a grand total
  const grandTotal = orderTotals.reduce((acc, val) => acc + val, 0);

  // Step 3: return value rounded to 2 decimals
